...
<head>
    ...
    <style>
        ...
        .hero-section {
            background: linear-gradient(rgba(0,0,0,.55), rgba(0,0,0,.55)), url("{{ asset('storage/img/store/suplemen.jpg') }}") center center / cover no-repeat;
           ...
           <a class="nav-link text-light" href="{{ route('home') }}">
            ...
            <div class="collapse navbar-collapse ms-auto tr" id="navbarNav">
                <ul class="navbar-nav ms-auto">
                    ...
                    <li class="nav-item mx-2"><a class="nav-link text-light-custom" href="{{ route('store') }}"style="pointer-events: none; opacity: 0.6; background-color: var(--primary); border-radius: 5px; ">Store</a></li>
...
<main>
    <section id="hero" class="hero-section">
        <div class="container hero-content">
            <div class="row">
                <div class="col-lg-8">
                    <h1 class="display-4 fw-bold mb-4">Kebutuhan dan Suplemen Olahraga<br><span class="text-primary-custom"></h1>
    ...
    <section id="about" class="py-5 bg-white" data-aos="fade-up">
        <div class="container">
        <h2 class="display-5 fw-bold text-center mb-5 text-dark">Store UPI GYM Fitness Centre</h2>
            <div class="row align-items-center g-5">
                @forelse($stores ?? [] as $store)
                <div class="col-md-4">
                     <div class="border-2 shadow-custom rounded-3 p-3 h-100">
                      <img src="{{ asset($store['image']) }}" alt="{{ $store['title'] }}" class="img-fluid rounded-3 shadow-custom">    
                        <h3 class="fs-4 fw-bold m-3">{{ $store['title'] }}</h3>
                        <p class="fs-6 mb-2 opacity-90">{{ $store['description'] }}</p>
                        <p class="fs-5 fw-bold text-primary-custom mb-3">Rp {{ number_format($store['price'], 0, ',', '.') }}</p>
                         <a href="#" class="btn btn-sm btn-primary-custom rounded">
                                            <i class="fas fa-store me-2"></i>Beli
                                        </a>
                    </div>
                </div>
                @empty
                <div class="col-12">
                    <p class="text-center text-muted">Toko sedang Maintenance</p>
                </div>
                @endforelse
            </div>
        </div>
